...
import com.example.toolgather.Adapter.adapter_hemo_v1.PhotographAdapter
import com.example.toolgather.Base.BaseActivity
import com.example.toolgather.R
import com.example.toolgather.Utils.GlideEngine
import com.example.toolgather.Utils.PhotographUtil
...
import kotlinx.android.synthetic.main.activity_photograph.*
...
class PhotographActivity : BaseActivity() {
    ...
    override fun initViews() {
        ...
        recycler?.layoutManager = GridLayoutManager(getContext(), 4)
            ...
            if (selectList.size > 0) {
                ...
                when (mediaType) {
                    ...
                    PictureConfig.TYPE_AUDIO ->                         // 预览音频
                        PictureSelector.create(this@PhotographActivity).externalPictureAudio(if (PictureMimeType.isContent(media.path)) media.androidQToPath else media.path)
                    else ->                         // 预览图片 可自定长按保存路径
                        PictureSelector.create(this@PhotographActivity)
