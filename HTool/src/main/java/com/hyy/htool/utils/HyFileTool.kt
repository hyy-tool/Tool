...
import com.hyy.htool.utils.HyDataTool.Companion.byte2FitSize
import com.hyy.htool.utils.HyDataTool.Companion.inputStream2Bytes
import com.hyy.htool.utils.HyDataTool.Companion.isNullString
import com.hyy.htool.utils.HyEncryptTool.encryptMD5File2String
import java.io.*
...
class HyFileTool {
    companion object {
        ...
        private const val TAG = "HyFileTool"// 取得sdcard文件路径
    ...
    """.trimIndent())
        ...
        fun writeFileFromIS(file: File?, `is`: InputStream?, append: Boolean): Boolean {
            ...
            return try {
                ...
                val data = ByteArray(HyConstTool.KB)
                var len: Int
                while (`is`.read(data, 0, HyConstTool.KB).also { len = it } != -1) {
                    os.write(data, 0, len)
                }
                true
            } catch (e: IOException) {
        ...
        fun getFileLines(file: File?): Int {
            ...
            try {
                `is` = BufferedInputStream(FileInputStream(file))
                val buffer = ByteArray(HyConstTool.KB)
                var readChars: Int
                while (`is`.read(buffer, 0, HyConstTool.KB).also { readChars = it } != -1) {
        ...
        fun getImageContentUri(context: Context?, imageFile: File?): Uri? {
            ...
            val cursor = context?.contentResolver?.query(
                MediaStore.Images.Media.EXTERNAL_CONTENT_URI, arrayOf(
                    MediaStore.Images.Media._ID), MediaStore.Images.Media.DATA + "=? ", arrayOf(filePath), null)
        ...
        fun getFilePhotoFromUri(context: Activity?, uri: Uri?): File {
            return File(HyPhotoTool.getImageAbsolutePath(context, uri))
        }
