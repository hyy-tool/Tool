 ...
 */
class HyCardStackView : ViewGroup, HyScrollDelegate {
    ...
    var rxScrollDelegate: HyScrollDelegate? = null
        private set
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) {
        val array = context.obtainStyledAttributes(attrs, R.styleable.HyCardStackView, defStyleAttr, defStyleRes)
        overlapGaps = array.getDimensionPixelSize(R.styleable.HyCardStackView_stackOverlapGaps, HyImageTool.dp2px(context, 20f))
        overlapGapsCollapse = array.getDimensionPixelSize(R.styleable.HyCardStackView_stackOverlapGapsCollapse, HyImageTool.dp2px(context, 20f))
        duration = array.getInt(R.styleable.HyCardStackView_stackDuration, HyAdapterAnimator.ANIMATION_DURATION)
        setAnimationType(array.getInt(R.styleable.HyCardStackView_stackAnimationType, UP_DOWN_STACK))
        numBottomShow = array.getInt(R.styleable.HyCardStackView_stackNumBottomShow, 3)
        array.recycle()
    ...
    fun setAdapter(rxAdapterStack: HyAdapterStack<*>?) {
        mHyAdapterStack = rxAdapterStack
        mHyAdapterStack!!.registerObserver(mObserver)
        refreshView()
    }

    fun setAnimationType(type: Int) {
        val rxAdapterAnimator: HyAdapterAnimator = when (type) {
            ALL_DOWN -> HyAdapterAllMoveDownAnimator(this)
            UP_DOWN -> HyAdapterUpDownAnimator(this)
            else -> HyAdapterUpDownStackAnimator(this)
        }
        setRxAdapterAnimator(rxAdapterAnimator)
    }

    fun setRxAdapterAnimator(rxAdapterAnimator: HyAdapterAnimator?) {
        clearScrollYAndTranslation()
        mHyAdapterAnimator = rxAdapterAnimator
        if (mHyAdapterAnimator is HyAdapterUpDownStackAnimator) {
            rxScrollDelegate = HyStackScrollDelegateImpl(this)
        } else {
    ...
    fun getViewHolder(i: Int): ViewHolder? {
        ...
        if (mViewHolders!!.size <= i || mViewHolders!![i].mItemViewType != mHyAdapterStack!!.getItemViewType(i)) {
            viewHolder = mHyAdapterStack!!.createView(this, mHyAdapterStack!!.getItemViewType(i))
    ...
    private fun doCardClickAnimation(viewHolder: ViewHolder?, position: Int) {
        checkContentHeightByParent()
        mHyAdapterAnimator!!.itemClick(viewHolder!!, position)
    }
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (activePointerIndex == -1) {
                    ...
                    if (mIsBeingDragged) {
                        ...
                        if (rxScrollDelegate is HyStackScrollDelegateImpl) {
                            (rxScrollDelegate as HyStackScrollDelegateImpl).scrollViewTo(0, deltaY + (rxScrollDelegate as HyStackScrollDelegateImpl).viewScrollY)
                        } else {
    ...
    var duration: Int
        get() = if (mHyAdapterAnimator != null) {
            mDuration
        } else 0
        set(duration) {
            mDuration = duration
    ...
    class LayoutParams : MarginLayoutParams {
        ...
        constructor(c: Context, attrs: AttributeSet?) : super(c, attrs) {
            @SuppressLint("CustomViewStyleable") val array = c.obtainStyledAttributes(attrs, R.styleable.HyCardStackView)
            mHeaderHeight = array.getDimensionPixelSize(R.styleable.HyCardStackView_stackHeaderHeight, -1)
            array.recycle()
        }
