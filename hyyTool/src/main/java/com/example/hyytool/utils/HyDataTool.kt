...
import com.example.hyytool.HyConstTool
import com.example.hyytool.HyConstTool.BYTE
import com.example.hyytool.HyConstTool.GB
import com.example.hyytool.HyConstTool.KB
import com.example.hyytool.HyConstTool.MB
import java.io.*
...
class HyDataTool {
    companion object {
        ...
        fun isContainSpace(value: String): Boolean {
            return value.contains(HyConstTool.SPACE)
        }
        ...
        fun formatCard(cardNo: String): String {
            ...
            var card = ""
            card = cardNo.substring(0, 4) + HyConstTool.SPACE + cardNo.substring(4, 8) + HyConstTool.SPACE + cardNo.substring(8, 12) + HyConstTool.SPACE
            card += cardNo.substring(cardNo.length - 4)
            return card
        }
         ...
         */
        @JvmStatic
        fun byte2Size(byteNum: Long, unit: HyConstTool.MemoryUnit?): Double {
            return if (byteNum < 0) {
                (-1).toDouble()
            } else when (unit) {
                HyConstTool.MemoryUnit.BYTE -> byteNum.toDouble() / BYTE
                HyConstTool.MemoryUnit.KB -> byteNum.toDouble() / KB
                HyConstTool.MemoryUnit.MB -> byteNum.toDouble() / MB
                HyConstTool.MemoryUnit.GB -> byteNum.toDouble() / GB
         ...
         */
        @JvmStatic
        fun size2Byte(size: Long, unit: HyConstTool.MemoryUnit?): Long {
            return if (size < 0) {
                -1
            } else when (unit) {
                HyConstTool.MemoryUnit.BYTE -> size * BYTE
                HyConstTool.MemoryUnit.KB -> size * KB
                HyConstTool.MemoryUnit.MB -> size * MB
                HyConstTool.MemoryUnit.GB -> size * GB
                else -> size * BYTE
            }
        }
        ...
        fun changeDistance(length: Double, displayMeter: Boolean): String {
            return if (length < 1000) {
                HyConstTool.FORMAT_TWO.format(length) + if (displayMeter) "米" else ""
            } else {
                HyConstTool.FORMAT_TWO.format(length / 1000) + if (displayMeter) "千米" else ""
            }
        }
